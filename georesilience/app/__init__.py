...
appbuilder = AppBuilder(
    indexview=MyIndexView, menu=Menu(reverse=True), base_template="base_template.html"
)

admin = Admin(
    name="GeoAssets",
    template_mode="bootstrap2",
    index_view=DashboardView(), #url='/geoassets', name="geoassets"),
    base_template="admin/custom_base.html",
    # endpoint="geoassets",  # Blueprint name for routing internally
    # url='/geoassets',  # Base URL for the web interface
    # static_url_path='static'  # Make sure this path is correct
)

admin_issues = Admin(
    name="InfraFeedback",
    template_mode="bootstrap2",
    index_view=DashboardView(url='/infrafeedback', name="infrafeedback"),
    base_template="admin/custom_base.html",
    endpoint="infrafeedback",  # Unique blueprint name
    url='/infrafeedback',  # Unique base URL
    static_url_path=None  # Avoid handling static files in the secondary admin
)
admin_issues.name = 'InfraFeedback'
...
def create_app():
    ...
    with app.app_context():
        ...
        admin.add_view(DashboardView(name="index"))
